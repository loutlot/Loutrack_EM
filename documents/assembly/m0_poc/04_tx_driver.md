...
| 3D11-722J | - | 1 | 送信コイル（1軸使用、SMD） |
| 3D11-722Jテストボード | 自作基板 | 1 | SMD→2.54mm変換基板 |
| ピンヘッダー 1×4（2.54mm） | 100167等 | 2 | テストボード用（上部・下部） |
...
### Step 5: コイル接続（3D11-722Jテストボード）

3D11-722Jは**SMD部品**のため、直接ブレッドボードに挿入できません。
**テストボード**を使用して2.54mmピッチのピンヘッダーに変換します。
...
3D11-722Jは3軸コイルですが、M0では**X軸のみ**使用します。
上部または下部のX軸端子を使用できます。

| DRV8835ピン | テストボード端子 | 備考 |
|-------------|------------------|------|
| AOUT1 | X+（上部または下部） | X軸プラス |
| AOUT2 | X-（上部または下部） | X軸マイナス |

💡 **テストボードの組み立て:**
- 詳細は [README.md](README.md) の「3D11-722Jテストボードについて」を参照
- 基板発注は [KiCadファイル](../../kicad/3D11_test_Board/) からGerberファイルを生成

⚠️ **注意:** テストボードの向きを確認してください（"UP"マークが上向き）。
