...
- **⭐ LIS3DH（3軸加速度センサー）**:
  - Hemispheric ambiguity解決用（詳細は10.3参照）
  - I2C接続（D4=SDA, D5=SCL）
  - ブレークアウトボード推奨（秋月/Adafruit）
...
### 10.3 Hemispheric Ambiguity解決

#### 10.3.1 問題

磁界測定では、+X+Y+Zの位置と-X-Y-Zの位置が同じ測定値を示す「Hemispheric ambiguity」が発生する。
...
**Kirbの講義**（`documents/outside_sources/Electromagnetic 6-DOF Tracking for VR Applications/reformatted_kirb_lecture.md`）を参考に、**IMU（加速度センサー）で重力方向を検出**して上下を識別する。

| 方法 | メリット | デメリット | 採用 |
|------|----------|------------|------|
| 位相検出 | IMU不要 | 位相同期が必要、実装複雑 | ❌ |
| 複数送信コイル | 追加コイルで検証 | コスト・配線増 | ❌ |
| **IMU併用** | シンプル、実証済み | センサー追加 | ⭐ 採用 |

#### 10.3.3 LIS3DHによる実装

- **センサー**: LIS3DH（3軸加速度センサー）
- **必要な機能**: 重力方向検出のみ → 低コストセンサーで十分
- **ジャイロ**: 不要（角速度は不要）
...
#### 10.3.4 初期キャリブレーション手順

1. **Beaconを設置し、向きを固定**（「下」方向を定義）
2. **Trackerを起動**、LIS3DHで重力方向を検出
3. **座標系を整列**: BeaconとTrackerの座標系を合わせる

#### 10.3.5 I/Qロックインとの関係

⚠️ **重要**: I/Qロックイン（I/Q同期検波）は**依然として必要**

| 目的 | I/Qロックイン | IMU (LIS3DH) |
|------|--------------|--------------|
| 信号検出・ノイズ除去 | ✅ 必須 | ❌ 不可 |
| 振幅A算出 | ✅ 必須 | ❌ 不可 |
| Hemispheric ambiguity | ❌ 使わない | ✅ これで解決 |

- 位相φも算出するが、**Hemispheric ambiguity解決には使わない**（IMUで解決）
- 位相同期の複雑な実装は不要
...
| **M1** | 3軸TDM（ディスクリート）+ **LIS3DH統合**: (Ax,Ay,Az)がスロットで正しく分離できる、軸干渉・過渡対策のパラメータ確立、**Hemispheric ambiguity解決** |
| **M2** | 3軸SMD試作基板（機能同等）: ディスクリートPoCと同等のA応答を再現、ノイズ・再現性の改善、LIS3DH SMD実装 |
